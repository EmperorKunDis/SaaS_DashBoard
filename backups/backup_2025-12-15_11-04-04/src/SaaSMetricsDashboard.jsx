...
export default function SaaSMetricsDashboard() {
  ...
  const metrics = useMemo(() => {
    ...
    for (let i = 0; i < months; i++) {
      const monthInput = monthlyInputs[i] || monthlyInputs[monthlyInputs.length - 1];
    ...
    // Let's show the Cumulative/Final state in the big cards, but maybe add a "This Month" breakdown?
    // The user asked to "zadat udaje" (enter data).
    // Let's keep the big cards showing the *End of Period* (Month 12) status, or maybe the *Active Month* status?
    // "Ted je ctvrty mesic" implies they are looking at Month 4.
    // Let's make the metrics reflect the *Active Month* state in the simulation?
    // No, usually you want to see the trajectory.
        ...
        <div className="xl:col-span-8 space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <MetricCard
              ...
              subtitle="Měsíční retence"
              color="emerald"
              small
            />
            <MetricCard
              label="Lifetime"
              value={`${metrics.avgLifetimeMonths} měs.`}
              subtitle="Průměrná doba"
              color="cyan"
              small
            />
            <MetricCard
              ...
              subtitle={metrics.ltvCacRatio >= 3 ? "✓ Zdravý" : metrics.ltvCacRatio >= 1 ? "⚠ Rizikový" : "✗ Kritický"}
              color={metrics.ltvCacRatio >= 3 ? "emerald" : metrics.ltvCacRatio >= 1 ? "amber" : "rose"}
              small
            />
            <MetricCard
              label="Zákazníci"
              value={metrics.totalCustomers.toLocaleString()}
              subtitle={`Měsíc ${metrics.cohortData.length}`}
              color="zinc"
              small
            />
          </div>

          {/* MRR & Customer Growth Chart */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* MRR Breakdown for Active Month */}
            <div className="bg-gradient-to-br from-zinc-900/80 to-zinc-900/40 rounded-2xl p-6 border border-zinc-800/50">
              <h3 className="text-lg font-semibold mb-4 flex justify-between items-center" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
                <span>📊 MRR Breakdown</span>
                <span className="text-xs text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded">Měsíc {activeMonth + 1}</span>
              </h3>
              <div className="space-y-4">
                <MRRBar
                  label="Nový MRR"
                  value={metrics.activeNewMRR}
                  maxValue={metrics.activeNewMRR + metrics.activeExpansionMRR}
                  color="emerald"
                />
                <MRRBar
                  label="Expansion MRR"
                  value={metrics.activeExpansionMRR}
                  maxValue={metrics.activeNewMRR + metrics.activeExpansionMRR}
                  color="violet"
                />
                <MRRBar
                  label="Lost MRR (Churn)"
                  value={metrics.activeLostMRR}
                  maxValue={metrics.activeNewMRR + metrics.activeExpansionMRR}
                  color="rose"
                  negative
                />
                <div className="pt-4 border-t border-zinc-700">
                  <div className="flex justify-between items-center">
                    <span className="text-zinc-400">Net MRR Change</span>
                    <span className={`text-xl font-bold ${(metrics.activeExpansionMRR - metrics.activeLostMRR + metrics.activeNewMRR) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      +{(metrics.activeNewMRR + metrics.activeExpansionMRR - metrics.activeLostMRR).toLocaleString()} Kč
                    </span>
                  </div>
                </div>
              </div>
